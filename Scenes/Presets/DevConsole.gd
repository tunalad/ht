...
func inventory(value: String = "") -> void:
	if value == "close":
		...
		inventory_do.emit("open")
	else:
		inventory_do.emit("toggle")


func backpack(value: String = "") -> void:
	if value == "show":
		backpack_do.emit("show")
	elif value == "hide":
		...
		if dir != null:
			if !song:
				...
				for file in files:
					if (
						file.ends_with(".tscn")
						or file.ends_with(".scn")
						or file.ends_with(".tscn.remap")
						or file.ends_with(".scn.remap")
					):
						var file_name := file.replace(".tscn", "").replace(".scn", "").replace(
							".remap", ""
						)
